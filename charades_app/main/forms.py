...
# from charades_app.models import Word, User, Category, GameInstance

class WordForm(FlaskForm):
    """Form to create a word"""
    name = StringField('Word',
        validators=[DataRequired(), Length(min=1, max=100)])
    hint = StringField('Hints for Word', Length(min=3, max=300))
    category = QuerySelectField('Category',
        query_factory=lambda: Category.query, allow_blank=False)
